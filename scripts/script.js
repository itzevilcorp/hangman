...
const getRandomWord = () => {
    ...
    currentWord = word; // Making currentWord as a random word
...
const gameOver = (isVictory) => {
    ...
    if (isVictory) {
        // Add 5 points to the score for the selected player
        const selectedPlayer = localStorage.getItem('selectedPlayer');
        let playerScore = parseInt(sessionStorage.getItem(`${selectedPlayer}_score`)) || 0;
        playerScore += 5;
        sessionStorage.setItem(`${selectedPlayer}_score`, playerScore);
    }

    // Redirect to the player menu after a brief delay (adjust as needed)
    setTimeout(() => {
